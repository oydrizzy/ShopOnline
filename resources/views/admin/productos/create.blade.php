...
<div class="max-w-3xl mx-auto px-4 py-10">
    ...
    <div class="bg-white shadow rounded-lg p-6">
       <h2 class="text-2xl font-bold mb-6 flex items-center gap-2 text-green-600">
        ...
        <form method="POST" action="{{ route('admin.productos.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input
                    ...
                    name="imagen"
                    accept="image/*"
                    class="w-full border-gray-300 rounded shadow-sm @error('imagen') border-red-500 @enderror"
                >
                @error('imagen')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex items-center gap-2 pt-3">
                <button type="submit" class="inline-flex items-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded hover:bg-green-700 transition">
                    Guardar
                </button>
